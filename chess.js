...
class ChessGame {
    constructor() {
        ...
        // Current piece set - validate that the saved piece set still exists
        const savedPieceSet = userPrefs?.pieceSet || currentPieceSet;
        // If the saved piece set no longer exists (was removed), fall back to unicode
    ...
    init() {
        ...
        if (!savedRoom) {
            ...
            if (savedColor && savedColor === 'black') {
                ...
                // White ALWAYS moves first in chess - don't change currentPlayer
                // currentPlayer stays 'white' - only the board orientation changes
        ...
        for (let displayRow = 0; displayRow < 8; displayRow++) {
            for (let displayCol = 0; displayCol < 8; displayCol++) {
                ...
                if (piece) {
                    ...
                    if (pieceSet && pieceSet[piece.color]) {
    ...
    applyColors() {
        ...
        squares.forEach(square => {
            ...
            if (boardPiece && pieceSet && pieceSet[boardPiece.color]) {
                if (!pieceSpan) {
